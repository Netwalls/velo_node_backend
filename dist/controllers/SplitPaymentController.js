...
exports.SplitPaymentController = void 0;
        ...
        try {
            ...
            try {
                ...
                if (/not supported by this library|RPC0\d\d/.test(msg)) {
                    console.warn(`⚠️ Skipping incompatible RPC (spec): ${endpoint} -> ${msg}`);
                    continue;
                }
            }
            ...
            console.warn(`⚠️ Failed to connect to ${endpoint}:`, err?.message);
            continue;
        }
    }
...
class SplitPaymentController {
    ...
    static async createSplitPayment(req, res) {
        try {
            const { title, description, chain, network, fromAddress, recipients } = req.body;
            // Validation
            ...
            const recipientRepo = database_1.AppDataSource.getRepository(SplitPaymentRecipient_1.SplitPaymentRecipient);
            const recipientRecords = normalizedRecipients.map((recipient) => recipientRepo.create({
    ...
    static async executeSplitPayment(req, res) {
        try {
            ...
            const feeCalculations = feeService_1.FeeService.calculateBatchFees(recipientAmounts);
            const totalFees = feeCalculations.reduce((sum, calc) => sum + calc.fee, 0);
            const totalPaymentAmount = parseFloat(splitPayment.totalAmount);
            const totalAmountWithFees = totalPaymentAmount + totalFees;
            console.log(`Split payment fee breakdown:
...
- Fee calculations:`, feeCalculations.map((calc, idx) => ({
            ...
            const treasuryWallet = treasury_1.default.getTreasuryWallet(splitPayment.chain, splitPayment.network);
            if (!treasuryWallet) {
                throw new Error(`Treasury wallet not configured for ${splitPayment.chain} on ${splitPayment.network}`);
            }
            console.log(`Treasury wallet for ${splitPayment.chain}/${splitPayment.network}: ${treasuryWallet}`);
            ...
            if (splitPayment.chain === "ethereum" ||
                ...
                results = await SplitPaymentController.processStarknetBatch(splitPayment, activeRecipients, privateKey);
            }
            else {
                throw new Error(`Chain ${splitPayment.chain} not supported for split payments`);
            }
            ...
            if (successCount > 0) {
                try {
                    console.log(`Executing fee collection: $${totalFees.toFixed(2)} to treasury...`);
                    ...
                    for (let i = 0; i < results.length; i++) {
                        if (results[i].success) {
                            ...
                            const feeTx = await feeCollectionService_1.default.createFeeTransferRecord({
                                userId: req.user.id,
                            ...
                            const feeRecord = await feeCollectionService_1.default.recordFee({
                                userId: req.user.id,
                            ...
                            console.log(`Fee recorded for ${activeRecipients[i].recipientAddress}: $${feeCalculations[i].fee.toFixed(2)} (${feeCalculations[i].tier})`);
                        }
                    }
    ...
    static async processStarknetBatch(splitPayment, recipients, privateKey) {
        const results = [];
    ...
    static async processStellarBatch(splitPayment, recipients, privateKey) {
        const results = [];
        ...
        try {
            ...
            try {
                ...
                throw new Error(`Failed to load Stellar account ${sourceKeypair.publicKey()} on ${network}: ${accountErr?.response?.data?.detail ||
                    accountErr?.message ||
                    String(accountErr)}`);
            }
            ...
            for (let i = 0; i < recipients.length; i++) {
                ...
                try {
                    ...
                    if (!destExists) {
                        ...
                        txBuilder.addOperation(SDK.Operation.createAccount({
                            destination: dest,
     ...
     */
    static async processPolkadotBatch(splitPayment, recipients, privateKey) {
        const results = [];
        if (!recipients || recipients.length === 0)
            return results;
        try {
            ...
            const totalTransferAmount = recipients.reduce((sum, recipient) => sum + Number(recipient.amount), 0);
            ...
            if (balanceDOT < totalNeeded) {
                throw new Error(`Insufficient balance. Available: ${balanceDOT} DOT, ` +
            ...
            for (let i = 0; i < recipients.length; i++) {
                ...
                try {
                    ...
                    console.log(`[Polkadot Split] [${i + 1}/${recipients.length}] ` +
                    ...
                    const txHash = await new Promise(async (resolve, reject) => {
                        try {
                            const unsub = await tx.signAndSend(sender, (result) => {
                                ...
                                if (status.isInBlock) {
                                    console.log(`[Polkadot Split] Transaction ${i + 1} included: ${status.asInBlock.toString()}`);
                                    resolve(status.asInBlock.toString());
                                    try {
                                        unsub();
                                    }
                                    ...
                                    console.log(`[Polkadot Split] Transaction ${i + 1} finalized: ${status.asFinalized.toString()}`);
                                    resolve(status.asFinalized.toString());
                                    try {
                                        unsub();
                                    }
                    ...
                    const errorMsg = err?.message || String(err);
                    console.error(`[Polkadot Split] ❌ Transfer ${i + 1} failed for ${recipient.recipientAddress}:`, errorMsg);
                    results.push({
                        success: false,
                        error: errorMsg,
                    });
        ...
        try {
            ...
            res.status(200).json({
                ...
                templates: splitPayments.map((sp) => ({
                    ...
                    canExecute: sp.status === SplitPayment_1.SplitPaymentStatus.ACTIVE,
                })),
    ...
    static async toggleSplitPayment(req, res) {
        try {
            ...
            // Toggle status
            splitPayment.status =
                splitPayment.status === SplitPayment_1.SplitPaymentStatus.ACTIVE
                    ? SplitPayment_1.SplitPaymentStatus.INACTIVE
                    : SplitPayment_1.SplitPaymentStatus.ACTIVE;
            await splitPaymentRepo.save(splitPayment);
            res.status(200).json({
                ...
                splitPayment: {
                    ...
                    canExecute: splitPayment.status === SplitPayment_1.SplitPaymentStatus.ACTIVE,
                },
            });
        }
        catch (error) {
    ...
    static async processBitcoinBatch(splitPayment, recipients, privateKey) {
        try {
            // Create one transaction with multiple outputs (true batch payment)
            const txHash = await SplitPaymentController.sendBitcoinBatchTransaction(splitPayment.fromAddress, recipients, splitPayment.network, privateKey);
    ...
    static async processSolanaBatch(splitPayment, recipients, privateKey) {
        ...
        try {
            ...
            const totalTransferAmount = recipients.reduce((sum, recipient) => sum + Math.round(Number(recipient.amount) * 1e9), 0);
            // Estimate fees (5000 lamports per signature is conservative)
            const estimatedFees = 5000 * recipients.length;
            const totalNeeded = totalTransferAmount + estimatedFees + rentExemptMinimum;
            if (senderBalance < totalNeeded) {
                throw new Error(`Insufficient balance. Available: ${senderBalance / 1e9} SOL, ` +
                    `Required: ${totalNeeded / 1e9} SOL (including transfers, fees, and rent)`);
            }
            // Process individual transactions instead of batch
            for (const recipient of recipients) {
                try {
                    const toPubkey = new web3_js_1.PublicKey(recipient.recipientAddress);
    ...
    static async sendBitcoinBatchTransaction(fromAddress, recipients, network, privateKey) {
        try {
            ...
            // Calculate total amount needed
            const totalTransferAmount = recipients.reduce((sum, recipient) => sum + Math.round(parseFloat(recipient.amount) * 100000000), 0);
            ...
            if (inputAmount < totalRequired) {
                throw new Error(`Insufficient balance. Required: ${totalRequired} satoshis (${totalRequired / 100000000} BTC), ` +
                    `Available: ${inputAmount} satoshis (${inputAmount / 100000000} BTC)`);
            }
            ...
            for (const utxo of selectedUtxos) {
                try {
                    ...
                    console.error(`Failed to get transaction data for UTXO ${utxo.txid}:`, error);
            ...
            for (const recipient of recipients) {
                ...
                if (amount < 546) {
                    ...
                    throw new Error(`Amount too small for recipient ${recipient.recipientAddress}: ` +
    ...
    static async sendBitcoinTransaction(fromAddress, toAddress, amount, network, privateKey) {
        ...
        return this.sendBitcoinBatchTransaction(fromAddress, recipients, network, privateKey);
    }
}
exports.SplitPaymentController = SplitPaymentController;
// Helper function
...
//# sourceMappingURL=SplitPaymentController.js.map