...
class AdminController {
    ...
    static async getStats(req, res) {
        try {
            ...
            const merchantRepo = database_1.AppDataSource.getRepository(MerchantPayment_1.MerchantPayment);
            const splitExecRepo = database_1.AppDataSource.getRepository(SplitPaymentExecution_1.SplitPaymentExecution);
            // Total users
            ...
            const txSumRaw = await txRepo
                .createQueryBuilder('t')
                .select('COALESCE(SUM(t.amount), 0)', 'sum')
                .where('t.status = :status', { status: 'confirmed' })
                .getRawOne();
            const merchantSumRaw = await merchantRepo
            ...
            const perChainRows = await txRepo
                .createQueryBuilder('t')
                .select('t.chain', 'chain')
                .addSelect('COUNT(*)', 'count')
                    ...
                    for (const a of allAddrs) {
                        ...
                        try {
                            ...
                            if (a.chain === 'ethereum' || a.chain === 'usdt_erc20') {
                                ...
                                const provider = new ethers.JsonRpcProvider(a.network === 'testnet'
                                ...
                                // @ts-ignore
                                const solWeb = await Promise.resolve().then(() => __importStar(require('@solana/web3.js')));
            ...
            let totalUSD = 0;
            const holdingsDetailed = [];
            const priceMap = await priceFeedService_1.PriceFeedService.getAllPrices();
            for (const [chainKey, balance] of Object.entries(holdingsByChain)) {
                ...
                try {
                    ...
                    if (cur) {
                        ...
                        if (!price || price === 0) {
                            try {
                                price = await priceFeedService_1.PriceFeedService.getPrice(cur);
            ...
            const merchantRows = await merchantRepo
                .createQueryBuilder('m')
            ...
            try {
                ...
                console.warn('query failed for split execution per-chain counts, continuing with empty results', err && (err.message || String(err)));
                splitRows = [];
            }
