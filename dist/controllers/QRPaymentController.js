...
function isValidStrkAddress(address) {
...
const STARKNET_TOKEN_CONTRACTS = {
    ...
    strk_usdt: {
        ...
        testnet: '0x068f5c6a61780768455de69077e07e89787839bf8166decfbf92b645209c0fb8',
    },
    strk_usdc: {
...
function padStarknetAddress(addr) {
    if (!addr)
        return addr;
...
function getBlockchainConfig(chain, network) {
    const normalizedNetwork = network?.toLowerCase() || 'mainnet';
    const chainConfig = BLOCKCHAIN_APIS[chain];
    if (!chainConfig) {
        throw new Error(`Unsupported chain: ${chain}`);
    }
    if (chain === 'usdt-erc20') {
        return BLOCKCHAIN_APIS.ethereum[normalizedNetwork];
    }
    const config = chainConfig[normalizedNetwork];
    if (!config) {
...
class MerchantController {
    ...
    static async createPayment(req, res) {
        try {
            const { amount, chain, network, stellarAddress, ethAddress, btcAddress, solAddress, strkAddress, polkadotAddress, usdtErc20Address, usdtTrc20Address, description, } = req.body;
            ...
            const payload = {
                ...
                usdtTrc20Address,
                address,
                status: MerchantPayment_1.MerchantPaymentStatus.PENDING,
                description: description ||
                    `Payment request for ${amount} ${chain.toUpperCase()}`,
            };
            const payment = (await paymentRepo.save(payload));
            // Create notification
            await database_1.AppDataSource.getRepository(Notification_1.Notification).save({
                userId,
                type: index_1.NotificationType.QR_PAYMENT_CREATED,
    ...
    static async cancelPayment(req, res) {
        try {
            ...
            payment.status = MerchantPayment_1.MerchantPaymentStatus.CANCELLED;
     ...
     */
    static async monitorPayment(req, res) {
        try {
            const userId = req.user.id;
            const { id } = req.params;
            const paymentRepo = database_1.AppDataSource.getRepository(MerchantPayment_1.MerchantPayment);
            ...
            if (payment.status !== MerchantPayment_1.MerchantPaymentStatus.PENDING) {
                return res.json({
                    payment,
                    blockchainStatus: {
                        confirmed: payment.status === MerchantPayment_1.MerchantPaymentStatus.COMPLETED,
                        transactionHash: payment.txHash || payment.transactionHash,
                        confirmations: payment.status === MerchantPayment_1.MerchantPaymentStatus.COMPLETED ? 1 : 0,
                    },
                });
            }
            ...
            if (blockchainStatus.confirmed &&
                payment.status === MerchantPayment_1.MerchantPaymentStatus.PENDING) {
     ...
     */
    static async monitorAllPendingPayments(req, res) {
        try {
            const paymentRepo = database_1.AppDataSource.getRepository(MerchantPayment_1.MerchantPayment);
     ...
     */
    static async checkBlockchainPayment(payment) {
        try {
    ...
    static isValidStellarAddress(addr) {
     ...
     */
    static async checkPolkadotPayment(payment) {
        try {
            const { decodeAddress } = require('@polkadot/util-crypto');
            ...
            try {
                ...
                if (SUBSCAN_API_KEY) {
                    ...
                    const resp = await axios_1.default.post(`${base}/api/scan/transfers`, { address: payment.address, direction: 'to', row: 50, page: 0 }, { headers: { 'Content-Type': 'application/json', 'X-API-Key': SUBSCAN_API_KEY }, timeout: 6000 });
                    const items = resp.data &&
                        typeof resp.data === 'object' &&
                        'data' in resp.data &&
                        Array.isArray(resp.data.data.transfers)
                        ? resp.data.data.transfers
                        : [];
            ...
            const provider = new WsProvider(wsUrl, 2000);
                ...
                try {
                    ...
                    for (let idx = 0; idx < events.length; idx++) {
                        ...
                        if (event.section !== 'balances')
                            continue;
                        if (event.method !== 'Transfer' && event.method !== 'TransferKeepAlive')
                            continue;
                        const dataAny = event.data;
                        if (!dataAny || dataAny.length < 3)
                            continue;
                        const to = dataAny[1];
                        const amt = dataAny[2];
                        let toId;
                        try {
                            toId = (to.toU8a ? to.toU8a() : new Uint8Array(to));
                        }
                        catch {
                            continue;
                        }
     ...
     */
    static async checkStellarPayment(payment) {
        try {
            ...
            const res = await axios_1.default.get(`${horizon}/accounts/${encodeURIComponent(address)}/payments`, {
     ...
     */
    static async checkEthereumPayment(payment) {
        try {
            const network = payment.network?.toLowerCase() || 'mainnet';
            const ethConfig = BLOCKCHAIN_APIS.ethereum[network];
            const apiKey = ethConfig.apiKey;
            const address = payment.address;
            // Query Etherscan API for transactions
            const response = await axios_1.default.get(ethConfig.apiUrl, {
                params: {
                    module: 'account',
                    action: 'txlist',
                    address: address,
     ...
     */
    static async checkBitcoinPayment(payment) {
        try {
            const address = payment.address;
            const network = (payment.network || 'mainnet').toLowerCase();
            // use Blockstream endpoints for mainnet/testnet
            const blockstreamBase = network === 'testnet'
                ? 'https://blockstream.info/testnet/api'
                : 'https://blockstream.info/api';
            const expectedSats = BigInt(Math.round(Number(payment.amount) * 1e8));
            ...
            let tipHeight = null;
            try {
                const tipResp = await axios_1.default.get(`${blockstreamBase}/blocks/tip/height`, { timeout: 8000 });
            ...
            const inspectTxForAddress = (tx) => {
                ...
                for (const out of outputs) {
                    const outAddr = out.scriptpubkey_address ??
                        ...
                        out.addr ??
                        null;
                    const value = BigInt(out.value ?? out.amount ?? out.satoshis ?? 0);
                    if (process.env.NODE_ENV !== 'production') {
                    ...
                    if (outAddr === address && value >= expectedSats) {
                        const blockHeight = tx.status?.block_height ??
                            ...
                            tx.blockheight ??
                            null;
                        const confirmed = blockHeight !== null && blockHeight !== undefined;
                        const confirmations = confirmed && tipHeight !== null
                            ? Math.max(0, tipHeight - Number(blockHeight) + 1)
                            : confirmed
                                ? 1
                                : 0;
                        return {
            ...
            let txs = [];
            try {
                const r = await axios_1.default.get(`${blockstreamBase}/address/${address}/txs`, { timeout: 10000 });
                txs = Array.isArray(r.data) ? r.data : [];
                if (process.env.NODE_ENV !== 'production') {
                    console.debug('[BTC] /txs returned count', txs.length, 'for', address);
                    if (txs.length > 0)
                        console.debug('[BTC] sample txid', txs[0].txid ?? txs[0].id ?? txs[0].hash);
                }
            }
            catch (e) {
                if (process.env.NODE_ENV !== 'production') {
                    console.error('[BTC] error fetching /txs', e && typeof e === 'object' && 'message' in e
                        ? e.message
                        : e);
                }
                txs = [];
            ...
            try {
                const rm = await axios_1.default.get(`${blockstreamBase}/address/${address}/txs/mempool`, { timeout: 8000 });
                const memTxs = Array.isArray(rm.data) ? rm.data : [];
                if (process.env.NODE_ENV !== 'production') {
                    console.debug('[BTC] /txs/mempool returned count', memTxs.length, 'for', address);
                }
                ...
                if (process.env.NODE_ENV !== 'production') {
                    console.error('[BTC] error fetching mempool', err && typeof err === 'object' && 'message' in err
                        ? err.message
                        : err);
                }
            ...
            try {
                const cfgUrl = BLOCKCHAIN_APIS.bitcoin[network].apiUrl || '';
                if (cfgUrl.includes('blockchain.info') ||
                    cfgUrl.includes('blockchain.info/rawaddr')) {
                    const resp = await axios_1.default.get(`${cfgUrl.replace(/\/rawaddr\/?$/, '')}/rawaddr/${address}`, { timeout: 10000 });
     ...
     */
    static async checkSolanaPayment(payment) {
        const clustersToTry = ['mainnet', 'devnet', 'testnet'];
        try {
            const wallet = payment.address;
            const expectedLamports = BigInt(Math.round(Number(payment.amount) * 1e9));
            ...
            for (const network of networks) {
                ...
                const fetchSignatures = async (target) => {
                    const res = await axios_1.default.post(rpc, {
                        jsonrpc: '2.0',
                        id: 1,
                ...
                const fetchTransaction = async (sig) => {
                    const res = await axios_1.default.post(rpc, {
                        jsonrpc: '2.0',
                        id: 1,
                ...
                if (!Array.isArray(sigsForWallet) ||
                    ...
                    try {
                        ...
                        const taList = taRes.data?.result?.value ?? [];
                        for (const ta of taList) {
                            if (ta && ta.pubkey)
                                targets.add(ta.pubkey);
                        }
                    }
                    catch {
                ...
                if (process.env.NODE_ENV !== 'production') {
                    console.debug('Solana monitor: trying network', network, 'targets', Array.from(targets));
                }
                // Iterate targets and look for matching transactions
                for (const target of Array.from(targets)) {
                    const sigs = await fetchSignatures(target).catch(() => []);
                    if (process.env.NODE_ENV !== 'production') {
                        console.debug('Solana monitor: signatures for target', target, sigs.length);
                    }
                    for (const sigObj of sigs) {
                        const sig = sigObj.signature;
                        const tx = await fetchTransaction(sig).catch(() => null);
                        ...
                        const accountKeys = Array.isArray(accountKeysRaw)
                            ? accountKeysRaw.map((k) => typeof k === 'string' ? k : k.pubkey)
                            : [];
                        ...
                        if (idx !== -1 &&
                            ...
                            const preBal = BigInt(tx.meta.preBalances[idx] ?? 0);
                            const postBal = BigInt(tx.meta.postBalances[idx] ?? 0);
                            ...
                            if (diff >= expectedLamports) {
                                ...
                                const confirmations = status === 'finalized'
                                    ? 32
                                    : status === 'confirmed'
                                        ? 1
                                        : 0;
                        ...
                        if (tx.meta.postTokenBalances &&
                            ...
                            try {
                                for (let i = 0; i < tx.meta.postTokenBalances.length; i++) {
                                    const postToken = tx.meta.postTokenBalances[i];
                                    const preToken = tx.meta.preTokenBalances[i];
                                    const owner = postToken && postToken.owner
                                        ? postToken.owner
                                        : null;
                                    if (owner !== wallet && owner !== target)
                                        continue;
                                    const preAmt = preToken &&
                                        preToken.uiTokenAmount &&
                                        preToken.uiTokenAmount.amount
                                        ? BigInt(preToken.uiTokenAmount.amount)
                                        : BigInt(0);
                                    const postAmt = postToken &&
                                        postToken.uiTokenAmount &&
                                        postToken.uiTokenAmount.amount
                                        ? BigInt(postToken.uiTokenAmount.amount)
                                        : BigInt(0);
                                    const tokenDiff = postAmt - preAmt;
                                    if (process.env.NODE_ENV !== 'production') {
                                        console.debug('Solana monitor token balances', {
                                            network,
                                            sig,
                                            owner,
                ...
                if (process.env.NODE_ENV !== 'production') {
                    console.debug('Solana monitor: no match on network', network);
                }
            } // end for networks
            if (process.env.NODE_ENV !== 'production') {
                console.debug('Solana monitor debug: no matching tx found for', {
                    address: payment.address,
                    amount: payment.amount,
 ...
 */
    static async checkStarknetPayment(payment) {
        try {
            const network = (payment.network || 'mainnet').toLowerCase();
            const recipientHex = padStarknetAddress(payment.address);
            const recipientDec = BigInt(recipientHex).toString();
            ...
            try {
                ...
                const response = await axios_1.default.get(`${voyagerBase}/contracts/${recipientHex}/transfers`, {
                    params: { ps: 100, p: 1 },
                    timeout: 15000,
                });
                ...
                for (const transfer of transfers) {
                    ...
                    if (toAddr !== recipientHex.toLowerCase())
                        continue;
                    let transferAmount;
                    try {
            ...
            for (const tokenContract of contracts) {
                try {
                    ...
                    const response = await axios_1.default.post(eventsUrl, {
                        contract_address: tokenContract,
                        from_block: 0,
                        to_block: 'latest',
                        keys: [
                    ...
                    if (!Array.isArray(events) || events.length === 0) {
                        const fallbackResponse = await axios_1.default.post(eventsUrl, {
                            contract_address: tokenContract,
                            from_block: 0,
                            to_block: 'latest',
                            page_size: 1000,
                    ...
                    for (const event of events) {
                        ...
                        if (eventRecipient !== recipientHex)
                            continue;
            ...
            try {
                ...
                const response = await axios_1.default.get(`${starkscanBase}/transactions`, {
                    params: {
                ...
                const txs = response.data && typeof response.data === 'object' && 'data' in response.data
                    ? response.data.data
                    : [];
                if (process.env.NODE_ENV !== 'production') {
    ...
    static async getPaymentStats(req, res) {
        try {
            const userId = req.user.id;
            const paymentRepo = database_1.AppDataSource.getRepository(MerchantPayment_1.MerchantPayment);
            const [totalPayments, pendingPayments, completedPayments, cancelledPayments,] = await Promise.all([
                paymentRepo.count({ where: { userId } }),
                paymentRepo.count({
                    where: { userId, status: MerchantPayment_1.MerchantPaymentStatus.PENDING },
                }),
                paymentRepo.count({
                    where: { userId, status: MerchantPayment_1.MerchantPaymentStatus.COMPLETED },
                }),
                paymentRepo.count({
                    where: { userId, status: MerchantPayment_1.MerchantPaymentStatus.CANCELLED },
                }),
            ]);
            // Calculate total amount by status
            const payments = await paymentRepo.find({ where: { userId } });
            const totalAmount = payments
                .filter((p) => p.status === MerchantPayment_1.MerchantPaymentStatus.COMPLETED)
...
//# sourceMappingURL=QRPaymentController.js.map