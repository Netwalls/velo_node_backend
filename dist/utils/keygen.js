...
exports.generateUsdcWallet = generateUsdcWallet;
...
function generateUsdcWallet() {
    ...
    // 2. Solana
    const solKeypair = web3_js_1.Keypair.generate();
    ...
    const starkPrivateKey = starknet_1.stark.randomAddress(); // Secure random private key
    const starkPublicKey = starknet_1.ec.starkCurve.getStarkKey(starkPrivateKey);
    const starkInfo = {
...
function generateStrkWallet(customPrivateKey) {
    ...
    let address = starknet_1.hash.calculateContractAddressFromHash(publicKey, classHash, constructorCallData, 0);
...
async function deployStrkWallet(provider, privateKey, publicKey, address, checkBalanceFirst = true) {
    ...
    try {
        ...
        return {
            account: new starknet_1.Account(provider, address, privateKey),
            transactionHash: "",
            contractAddress: "",
        };
    }
