...
exports.PaymentMonitoringService = void 0;
...
const MerchantPayment_2 = require("../entities/MerchantPayment");
...
class PaymentMonitoringService {
     ...
     */
    static async checkForIncomingPayments() {
        try {
            console.log('Checking for incoming payments...');
            const merchantPaymentRepo = database_1.AppDataSource.getRepository(MerchantPayment_1.MerchantPayment);
            const pendingPayments = await merchantPaymentRepo.find({
                where: { status: MerchantPayment_2.MerchantPaymentStatus.PENDING },
            });
            for (const payment of pendingPayments) {
                ...
                if (detected &&
                    ...
                    console.log(`Merchant payment ${payment.id} marked as completed.`);
                }
            }
     ...
     */
    static async scanStellarAddressesForDeposits() {
        try {
     ...
     */
    static async scanAllAddressesForDeposits() {
        try {
            const addrRepo = database_1.AppDataSource.getRepository(UserAddress_1.UserAddress);
            const txRepo = database_1.AppDataSource.getRepository(Transaction_1.Transaction);
     ...
     */
    static async processIncomingPayment(payment) {
        try {
            ...
            if (!(await this.validatePayment(payment))) {
                console.log(`Payment validation failed for transaction ${payment.txHash}`);
                return;
            }
            ...
            const conversion = await conversionService_1.ConversionService.processAutomaticConversion(user.id, payment.currency, payment.amount, payment.fromAddress, payment.txHash);
     ...
     */
    static async validatePayment(payment) {
        try {
     ...
     */
    static getMinConfirmations(currency) {
     ...
     */
    static async simulatePaymentDetection(payment) {
        console.log('Simulating payment detection:', payment);
        await this.processIncomingPayment(payment);
    }
}
exports.PaymentMonitoringService = PaymentMonitoringService;
PaymentMonitoringService.isRunning = false;
PaymentMonitoringService.monitoringInterval = null;
 ...
 */
async function checkBlockchainForDeposit(chain, network, address, amount) {
    // ETHEREUM (ETH)
    ...
    if (chain.toLowerCase() === 'solana' || chain.toLowerCase() === 'sol') {
        ...
        try {
            ...
            const { data } = await axios_1.default.post(endpoint, body);
            const solanaData = data;
            ...
            for (const sig of solanaData.result) {
                ...
                const txData = await axios_1.default.post(endpoint, txBody);
                const txInfo = txData.data.result;
                if (!txInfo)
                    continue;
                // Find the index of your address in accountKeys
                const accountKeys = txInfo.transaction?.message?.accountKeys || [];
                const idx = accountKeys.findIndex((k) => (typeof k === 'string' ? k : k.pubkey) === address);
                if (idx === -1)
                    continue;
...
//# sourceMappingURL=paymentMonitoringService.js.map