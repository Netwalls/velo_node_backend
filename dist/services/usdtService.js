...
class USDTService {
     ...
     */
    static async getUserUSDTAddresses(userId) {
        try {
            const addressRepository = database_1.AppDataSource.getRepository(UserAddress_1.UserAddress);
            const addresses = await addressRepository.find({
                where: {
                    userId,
                    chain: (0, typeorm_1.In)([types_1.ChainType.USDT_ERC20, types_1.ChainType.USDT_TRC20]),
                },
            });
            const result = {
            ...
            addresses.forEach((addr) => {
                if (addr.chain === types_1.ChainType.USDT_ERC20) {
                    if (addr.network === types_1.NetworkType.MAINNET) {
                        result.erc20.mainnet = addr.address || '';
                    }
                    else if (addr.network === types_1.NetworkType.TESTNET) {
                        result.erc20.testnet = addr.address || '';
                    }
                }
                else if (addr.chain === types_1.ChainType.USDT_TRC20) {
                    if (addr.network === types_1.NetworkType.MAINNET) {
     ...
     */
    static async getUSDTBalanceFromBlockchain(userId) {
        try {
            ...
            const balances = {
                erc20: {
                    mainnet: await this.getERC20Balance(addresses.erc20.mainnet, 'mainnet'),
                    testnet: await this.getERC20Balance(addresses.erc20.testnet, 'testnet'),
                },
                trc20: {
                    mainnet: await this.getTRC20Balance(addresses.trc20.mainnet, 'mainnet'),
                    testnet: await this.getTRC20Balance(addresses.trc20.testnet, 'testnet'),
                },
            };
            const total = balances.erc20.mainnet +
     ...
     */
    static async getERC20Balance(address, network) {
        try {
            if (!address)
                return 0;
            ...
            console.log(`[SIMULATION] Getting ERC-20 USDT balance for ${address} on ${network}`);
     ...
     */
    static async getTRC20Balance(address, network) {
        try {
            if (!address)
                return 0;
            ...
            console.log(`[SIMULATION] Getting TRC-20 USDT balance for ${address} on ${network}`);
            return 0; // Simulated balance
        }
        catch (error) {
     ...
     */
    static async transferUSDTToUser(userId, amount, network = 'erc20') {
        try {
            const addresses = await this.getUserUSDTAddresses(userId);
            const targetAddress = network === 'erc20'
                ? addresses.erc20.mainnet
                : addresses.trc20.mainnet;
            if (!targetAddress) {
                throw new Error(`No ${network.toUpperCase()} USDT address found for user`);
            }
            ...
            console.log(`[SIMULATION] Transferring ${amount} USDT via ${network.toUpperCase()} to ${targetAddress}`);
     ...
     */
    static async updateDatabaseBalance(userId, amount) {
        try {
            ...
            console.log(`[DATABASE] Updated USDT balance for user ${userId}: ${user.usdtBalance}`);
        }
        catch (error) {
     ...
     */
    static generateSimulatedTxHash(network) {
     ...
     */
    static async getPrimaryUSDTAddress(userId) {
        try {
            const addresses = await this.getUserUSDTAddresses(userId);
...
//# sourceMappingURL=usdtService.js.map