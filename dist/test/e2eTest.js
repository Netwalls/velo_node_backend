...
class E2ETestRunner {
        ...
        try {
            ...
            const mockPayment = {
                txHash: '0x' +
                    ...
                    Math.random().toString(16).substring(2),
                fromAddress: '0x742d35cc6bf4532c32cf0e0d9b0b7c8f8' +
            ...
            await paymentMonitoringService_1.PaymentMonitoringService.simulatePaymentDetection(mockPayment);
            // Wait a moment for processing
            await new Promise((resolve) => setTimeout(resolve, 2000));
            // Step 7: Check final balance
            console.log('\n💰 Step 7: Checking final balance...');
            const finalBalance = await conversionService_1.ConversionService.getUSDTBalance(userId);
            console.log(`Final USDT balance: ${finalBalance}`);
            // Step 8: Get conversion history
            console.log('\n📜 Step 8: Getting conversion history...');
            const history = await conversionService_1.ConversionService.getConversionHistory(userId, 1, 5);
            console.log(`Recent conversions (${history.total} total):`);
            history.conversions.forEach((conversion, index) => {
                console.log(`  ${index + 1}. ${conversion.inputAmount} ${conversion.fromCurrency} → ${conversion.outputAmount} ${conversion.toCurrency} (${conversion.status})`);
            });
            // Step 9: Get blockchain balance
            console.log('\n🌐 Step 9: Getting blockchain USDT balance...');
            const blockchainBalance = await usdtService_1.USDTService.getUSDTBalanceFromBlockchain(userId);
            console.log('Blockchain balances:', blockchainBalance);
            console.log('\n✅ End-to-End Test Completed Successfully!');
            console.log('==========================================');
            console.log(`💎 Balance increased by: ${finalBalance - initialBalance} USDT`);
            console.log(`🔄 ETH → USDT conversion: ${ethAmount} ETH → ${calculation.outputAmount} USDT`);
        }
        catch (error) {
     ...
     */
    static async runManualConversionTest(userId) {
        console.log('\n🔧 Starting Manual Conversion Test');
        console.log('==================================');
        try {
            // Step 1: Get conversion rates
            console.log('\n📊 Step 1: Getting conversion rates...');
            const rates = await priceFeedService_1.PriceFeedService.getAllPrices();
            ...
            const calculation = await priceFeedService_1.PriceFeedService.calculateConversion(btcAmount, 'BTC', 'USDT');
            console.log('Manual conversion calculation:', calculation);
            // Step 3: Execute manual conversion
            console.log('\n⚡ Step 3: Executing manual conversion...');
            const conversion = await conversionService_1.ConversionService.processManualConversion(userId, 'BTC', 'USDT', btcAmount, 'bc1q' + Math.random().toString(16).substring(2, 10) // Mock BTC address
            );
    ...
    static async runPriceFeedTest() {
        ...
        try {
            ...
            const solPrice = await priceFeedService_1.PriceFeedService.getPrice('SOL');
            console.log(`ETH: $${ethPrice}`);
            console.log(`BTC: $${btcPrice}`);
            console.log(`SOL: $${solPrice}`);
            // Test conversion rate calculation
            console.log('\n🔄 Testing conversion rate calculation...');
            const ethToUsdt = await priceFeedService_1.PriceFeedService.getConversionRate('ETH', 'USDT');
            const btcToUsdt = await priceFeedService_1.PriceFeedService.getConversionRate('BTC', 'USDT');
            ...
            const allPrices = await priceFeedService_1.PriceFeedService.getAllPrices();
