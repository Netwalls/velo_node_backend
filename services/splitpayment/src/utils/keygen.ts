...
export async function checkBalance(
    ...
    try {
        ...
        const strkRes: any = await provider.callContract({
            contractAddress: strkTokenAddress,
            entrypoint: 'balanceOf',
            calldata: [address],
        }, 'latest'); // Use 'latest' instead of 'pending'

        // Handle Starknet v5 response structure (contains 'result' array)
        const strkVal = strkRes.result ? strkRes.result[0] : (Array.isArray(strkRes) ? strkRes[0] : '0');
        const strkBalance = BigInt(strkVal || '0');
        ...
        const ethRes: any = await provider.callContract({
            contractAddress: ethTokenAddress,
            entrypoint: 'balanceOf',
            calldata: [address],
        }, 'latest'); // Use 'latest' instead of 'pending'

        const ethVal = ethRes.result ? ethRes.result[0] : (Array.isArray(ethRes) ? ethRes[0] : '0');
        const ethBalance = BigInt(ethVal || '0');
