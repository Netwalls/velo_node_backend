  ...
  try {
    ...
    try {
      const verifyResult = await electricityService.verifyMeterNumber(
        ...
        "04172719595" // Test meter number
      );
      console.log("✅ Meter verification result:", verifyResult);
    } catch (verifyError: any) {
      console.log(
        "⚠️ Meter verification test skipped (expected in test environment):",
        verifyError.message
      );
    }
    console.log("");
    ...
    try {
      const purchaseResult = await electricityService.processElectricityPayment(
        userId,
        {
          type: "electricity",
          amount: testAmount,
          chain: Blockchain.SOLANA,
          company: ElectricityCompany.KADUNA_ELECTRIC,
          meterType: MeterType.PREPAID,
          meterNumber: "04172719595",
          phoneNumber: "2349125381992",
          transactionHash:
            "21dUmM8njMsjjbDzhvbgLoT2PeNTrJRzZ9gZQDGoPKgnf18frYGYph2ueKdBu4ye1ihW2wzwqfNGJBqNZ5e7868m",
        }
      );
      console.log(
        "✅ Purchase result:",
      ...
      console.log(
        "⚠️ Purchase test skipped (expected in test environment):",
    ...
    const history = await electricityService.getUserElectricityHistory(
      userId,
      10
    );
    ...
    history.forEach((payment, index) => {
      console.log(
        `   ${index + 1}. ${payment.company}: ₦${payment.fiat_amount} - ${
          payment.status
        }`
      );
    ...
    console.log("\n--- Ikeja Electric ---");
    const ikejaConfig = companies.find(
      (c) => c.value === ElectricityCompany.IKEJA_ELECTRIC
    );
    console.log(
      `${ikejaConfig?.label}: ₦${ikejaConfig?.minAmount} - ₦${ikejaConfig?.maxAmount}`
    );

    console.log("\n--- Ibadan Electric ---");
    const ibadanConfig = companies.find(
      (c) => c.value === ElectricityCompany.IBADAN_ELECTRIC
    );
    console.log(
      `${ibadanConfig?.label}: ₦${ibadanConfig?.minAmount} - ₦${ibadanConfig?.maxAmount}`
    );
    ...
    console.log(
      `   - Crypto Amount: ${expectedAmount.cryptoAmount} ${expectedAmount.cryptoCurrency}`
    );
    ...
    console.log(
      "API Key starts with:",
      process.env.NELLOBYTES_API_KEY?.substring(0, 6)
    );
  } catch (error: any) {
