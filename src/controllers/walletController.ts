...
`);
      ...
      // POLKADOT - Fixed version
        ...
        try {
          ...
          try {
            ...
            console.log(
              `[DEBUG] Account balance: ${balance} Planck (${
                Number(balance) / 1e10
              } DOT)`
            );
      ...
      // BTC
        ...
        const feeSats = Math.round(
          (chain === "bitcoin" ? feeTokenRounded : 0) * 1e8
        );
        ...
        if (inputSum < targetAmount + estimatedFee) {
          throw new Error(
            `Insufficient balance. Have: ${inputSum / 1e8} BTC, Need: ${
      ...
      (async () => {
        ...
        try {
          await FeeCollectionService.recordFee({
            ...
            description: `Transaction fee for sending ${
              feeCalculation.recipientReceives
          ...
          if (addr.chain === "starknet") {
            try {
              ...
              if (!providersMain[key])
                providersMain[key] = new RpcProvider({
                  nodeUrl: `https://starknet-mainnet.g.alchemy.com/starknet/version/rpc/v0_9/${process.env.ALCHEMY_STARKNET_KEY}`,
                });
              const provider = providersMain[key];
              const strkTokenAddress =
                "0x04718f5a0fc34cc1af16a1cdee98ffb20c31f5cd61d6ab07201858f4287c938d";
              const paddedAddress = padStarknetAddress(addr.address as string);
              ...
              console.warn(
                "Starknet balance fetch failed for",
                addr.address,
                strkErr?.message || String(strkErr)
              );
              throw strkErr;
            }
          } else if (addr.chain === "ethereum") {
            ...
            try {
              ...
              // Handle 404 (account not found) and other errors gracefully
              if ((stellarErr as any)?.response?.status === 404) {
                console.warn("Stellar account not found for", addr.address);
                return {
                  chain: addr.chain,
                  network: "mainnet",
                  address: addr.address,
              ...
              console.warn(
                ...
                addr.address,
                stellarErr?.message || String(stellarErr)
              );
              throw stellarErr;
            }
          } else if (addr.chain === "polkadot") {
