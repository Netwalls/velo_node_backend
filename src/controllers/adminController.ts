...
export class AdminController {
  ...
  static async getStats(req: Request, res: Response) {
    try {
      ...
      const merchantSum =
        merchantSumRaw && merchantSumRaw.sum ? Number(merchantSumRaw.sum) : 0;
      const splitExecSum =
        splitExecSumRaw && splitExecSumRaw.sum
      ...
      const perChainRows: any[] = await txRepo
        ...
        .addSelect('COUNT(*)', 'count')
        .addSelect(
          'COALESCE(SUM(CASE WHEN t.status = :status THEN t.amount ELSE 0 END), 0)',
          'confirmed_sum'
        )
        ...
        (async () => {
          ...
          for (const a of allAddrs) {
            ...
            try {
              ...
              console.warn(
                `Background: Failed to fetch balance for ${a.chain} ${a.address}:`,
            ...
            try {
              ...
              console.warn(
                'Background: Failed to persist lastKnownBalance for address',
                a.address,
                (saveErr as any) &&
                  ((saveErr as any).message || String(saveErr))
              );
            }
          }
          console.log('[Admin] Background balance refresh completed');
        })().catch((e) =>
          console.error('[Admin] Background refresh error:', e)
        );
      }
      ...
      try {
        ...
        console.warn(
          'query failed for split execution per-chain counts, continuing with empty results',
          (err as any) && ((err as any).message || String(err))
        );
        splitRows = [];
      }
      ...
      const depositNotifCount = await notificationRepo.count({
        where: { type: NotificationType.DEPOSIT },
      });
      ...
      for (const [chain, vals] of Object.entries(activityMap)) {
        if (vals.total > maxActivity) {
          ...
          const currentHoldings =
            holdingsDetailed.find((h) => h.chain === computedMostUsed)?.usd ||
            0;
          const challengerHoldings =
            holdingsDetailed.find((h) => h.chain === chain)?.usd || 0;
