          ...
          try {
            ...
            if (balanceDecimal > 0 && addr.encryptedPrivateKey) {
              try {
                ...
                console.log(
                  `[DEBUG] Checking if ${addr.address} can be deployed...`
                );
                ...
                if (hasSufficientFunds) {
                  console.log(
                    `[DEBUG] Deploying Starknet account ${addr.address}...`
                  );
                  ...
                  console.log(
                    `[SUCCESS] Starknet account ${addr.address} deployed successfully`
                  );
                  ...
                  console.log(
                    `[DEBUG] Insufficient funds for deployment. Balance: ${balance}`
                  );
                }
              }
            }
      ...
      for (const addr of addresses) {
        if (addr.chain === 'starknet') {
          try {
            ...
            const strkBalanceHex =
              strkResult && strkResult[0] ? strkResult[0] : '0x0';
            const strkBalanceDecimal = parseInt(strkBalanceHex, 16);
            const balanceInSTRK = (strkBalanceDecimal / 1e18).toString();

            console.log(
              `[DEBUG] Mainnet ${addr.address}: ${balanceInSTRK} STRK`
            );
            ...
            if (strkBalanceDecimal > 0 && addr.encryptedPrivateKey) {
              try {
                ...
                console.log(
                  `[DEBUG] Account ${
                    addr.address as string
                  } already deployed (classHash: ${classHash})`
                );
                ...
                try {
                  ...
                  console.log(
                    '[DEBUG] Performing INLINE balance check (STRK-first)...'
                  );
                  const strkTokenAddr =
                    '0x04718f5a0fc34cc1af16a1cdee98ffb20c31f5cd61d6ab07201858f4287c938d';
                  const ethTokenAddr =
                  ...
                  if (strkBal >= minBal) {
                    ...
                    console.log(
                      `[DEBUG] INLINE ETH Balance: ${Number(ethBal) / 1e18} ETH`
                    );
                  ...
                  console.log(
                    `  - Current Balance: ${balanceFormatted} ${token}`
                  );
                  console.log(`  - Required Balance: 0.5 ${token}`);

                  if (hasSufficientFunds) {
                    console.log(
                      `[DEBUG] ✅ Starting deployment with ${token} fee token...`
                    );
                    ...
                    console.log(
                      `[DEBUG] Private key decrypted, public key: ${publicKey}`
                    );

                    if (!addr.address) throw new Error('Missing user address.');
                    ...
                    const contractAddr =
                      deployResult.contractAddress || addr.address;
                    ...
                    console.log(
                      `[WARNING] ❌ Insufficient ${token} for deployment`
                    );
                    console.log(`  Current: ${balanceFormatted} ${token}`);
                    console.log(`  Required: 0.5 ${token} minimum`);
                    console.log(
                      `  💡 You have ${balanceInSTRK} STRK but need ${
                        token === 'ETH' ? 'ETH' : 'more STRK'
                      } to deploy`
                    );
                  ...
                  if (deployError?.message) {
                    console.error(
                      `[ERROR] Error message: ${deployError.message}`
                    );
                  }
              ...
              console.log(
                `[DEBUG] No private key available for ${addr.address} - cannot deploy`
              );
            }
          } catch (error: any) {
            console.error(
              'Starknet mainnet balance/deployment error:',
              error?.message || error
            );
            balances.push({
              chain: addr.chain,
