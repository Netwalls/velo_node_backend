...
export class PaymentMonitoringService {
  ...
  private static async checkForIncomingPayments(): Promise<void> {
    try {
      ...
      const merchantPaymentRepo = AppDataSource.getRepository(MerchantPayment);
      const pendingPayments = await merchantPaymentRepo.find({
        where: { status: MerchantPaymentStatus.PENDING },
      });

      for (const payment of pendingPayments) {
        if (!payment.chain || !payment.network || !payment.address) continue;
  ...
  private static async scanStellarAddressesForDeposits(): Promise<void> {
    try {
      ...
      const stellarAddresses = await addrRepo.find({
        where: { chain: ChainType.STELLAR },
      });
      for (const addr of stellarAddresses) {
        try {
          const horizon =
            (addr.network || 'mainnet').toLowerCase() === 'testnet'
          ...
          const url = `${horizon}/accounts/${encodeURIComponent(
            addr.address
          )}/payments`;
          ...
          console.debug(
            `Stellar scan failed for ${addr.address}:`,
   ...
   */
  static async processIncomingPayment(payment: IncomingPayment): Promise<void> {
    try {
      ...
      const conversion = await ConversionService.processAutomaticConversion(
        user.id!,
        payment.currency,
        payment.amount,
        payment.fromAddress,
...
async function checkBlockchainForDeposit(
  ...
  if (chain.toLowerCase() === 'ethereum' || chain.toLowerCase() === 'eth') {
    ...
    try {
      ...
      if (ethData.status !== '1' || !Array.isArray(ethData.result)) return null;
    ...
    try {
      ...
      for (const sig of solanaData.result) {
        ...
        const idx = accountKeys.findIndex(
          (k: any) => (typeof k === 'string' ? k : k.pubkey) === address
        );
        if (idx === -1) continue;
